...
- one initContainer and one sidecar in each Pod to assemble and refresh local RankTable files.
...
2. **initContainer (Pod startup path)**  
   Reads mounted index metadata, downloads required shards from apiserver, validates, assembles, decompresses, writes local RankTable file atomically.

3. **sidecar (runtime refresh path)**  
   Watches mounted index file updates and reconciles local RankTable to the latest complete version.

4. **business container**  
   Reads only local file generated by init/sidecar.

### Data flow

1. Controller publishes shard ConfigMaps first.
2. Controller publishes index ConfigMap with `status=completed`.
3. initContainer/sidecar sees index update, then fetches shards via apiserver `GET` calls.
...
### initContainer (startup)

1. Load and validate index schema.
2. Ensure `status=completed`.
3. Build shard fetch plan.
4. Download shards (bounded worker pool).
5. Validate each shard (`size`, `sha256`).
6. Concatenate by `id`.
7. Validate merged compressed stream (`compressed_sha256`, `compressed_size`).
8. Decompress based on `encoding` using a **streaming decode with a hard output cap** of  
   `min(original_size, max_original_size if set, runtime max-bytes flag)` so gzip/zstd cannot expand unbounded in memory before validation (mitigates “zip bombs”).
9. Validate final content (`content_sha256`, `original_size`, optional `max_original_size`).
10. Atomic write (`tmp + fsync + rename`) to target file.

### sidecar (runtime)

Same reconcile pipeline as initContainer plus:

- skip if version unchanged,
- single in-flight reconcile (deduplicate concurrent triggers),
- on failure keep previous valid local file; the **next** reconcile may be driven by the periodic poll or a new index event (reference binary logs errors; shard `GET` uses bounded exponential backoff + jitter on transient API errors).
...
7. **Version idempotency**: never reconcile same version twice.
...
- Container image: build from source binary and deploy as both initContainer and sidecar.
...
4. **E2E tests** (recommended new suite: `test/e2e/ranktable/`)
