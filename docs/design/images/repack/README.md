...
| `repack-controller-architecture.svg` | **repack-controller 架构与职责**（Volcano 风格）：左=apiserver(RepackRun/RepackPolicy P1/替身 Pod)→中=控制器三个 reconciler（① RunGC·TTL、② 提名 reconciler、③ RepackPolicy 生成 P1）+ `pkg/state` 纯函数→右=协作产出（引擎写 nominations、patch pod.nominatedNodeName、scheduler honor、DELETE/CREATE Run）；底部收纳边界(不做准入/不做驱逐)、部署(shim 或独立)、配置 | repack-controller |
