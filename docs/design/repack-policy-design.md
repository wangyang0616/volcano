...
**选择单元 = PodGroup（更通用）**：scope 圈选的对象是 **PodGroup**，而非特指 Volcano Job（vcjob）。理由是 **Volcano 调度引擎本身就以 PodGroup 为单元**——`api.JobInfo.UID = "<podgroup.namespace>/<podgroup.name>"`（`cache/event_handlers.go::getJobID(pg)`），`ValidatePlan` / gang 判定 / victim 选择全在 PodGroup 粒度。
...
| **`> 0`** | 到达终态（`Succeeded` / `Failed` / `Cancelled`）且 `status.completionTime` 已设后，**`completionTime + TTL`** 到期由 **RunGC** DELETE |
...
| **`Cancelled`** | volcano-repack-engine / Controller | 用户 **主动取消** 或 DELETE 收尾 |

**刻意不采用的 phase**

| 不采用 | 原因 | 替代 |
|--------|------|------|
| `Queued` 独立 phase | 与 Pending 边界模糊 | phase 保持 **`Pending`**，`conditions` 标 `Queued` |
...
| `Running → Cancelled` | **volcano-repack-engine** | 收到删除/取消信号，best-effort 停引擎 |

**Succeeded / Failed 判定（结合业务能力）**

| 场景 | phase | 典型 `conditions.reason` |
|------|-------|--------------------------|
| DryRun 模拟完成 | **Succeeded** | `RepackRecommended`（有方案）/ `NoFragmentation` / `BelowGoalThreshold`（无收益） |
| Execute 完成且驱逐流程正常结束（含 **0 条** `plan.moves`） | **Succeeded** | `Executed`（有搬迁）/ `NoFragmentation` / `BelowGoalThreshold`（空操作） |
| scope 非法（mode=Execute 无 scope、goals>1 等） | **CEL 创建期拒绝**（对象不落库，非 Failed） | — |
| 崩溃孤儿（引擎重启时遗留 Running） | **Failed** | `Interrupted` |
| 引擎 panic / informer 不可用 | **Failed** | `EngineError` |
| Eviction API 硬失败（权限、冲突不可恢复） | **Failed** | `EvictionFailed` |
| 用户 `kubectl delete` 执行中 Run | **Cancelled** | `CancelledByUser` |

> **与 Job 的差异**：Job 以 Pod 失败计数判 Failed；Repack **无副本**，「scope 内无可行搬迁」仍算 **Succeeded**（结果写在 `status.plan`，`moves` 可能为空，`conditions[Complete].reason` 为 `NoFragmentation`/`BelowGoalThreshold`），避免把「业务上无可做」误标成系统故障。

**`status.conditions`（对齐 Job 风格）**

| type | 何时 True | 典型 reason | 备注 |
|------|-----------|-------------|------|
| **`Queued`** | phase=`Pending` 且暂不能认领 | `AnotherRunActive` / `ExecuteCoolingDown` / `WaitingForLeader` | **Repack 特有**；K=1 占用 / Execute 冷静期 / Leader 切换（**仅 Execute 受前两者门控，DryRun 不排此队**）。准入=CEL，无 `Admitted` 条件 |
| **`Progressing`** | phase=`Running` | `Simulating`（DryRun）/ `Evicting`（Execute） | 子阶段用 **reason** 区分，不增 phase |
| **`Complete`** | phase=`Succeeded` | `RepackRecommended` / `Executed` / `NoFragmentation` / `BelowGoalThreshold` | 对齐 Job `Complete`；reason 兼任「值不值得整理」收口。引擎内部「收益门控不达标」的 `NoRepackNeeded` 分支即映射为 `NoFragmentation`/`BelowGoalThreshold` |
| **`Failed`** | phase=`Failed` | 见上表 | 对齐 Job `Failed` |
| **`Cancelled`** | phase=`Cancelled` | `CancelledByUser` | — |
...
| `observedGeneration` | Controller | 对齐 K8s 惯例；Admit 后观测到的 `metadata.generation` |
| `triggerReason` | Controller | 创建原因：`Manual` / `OnPendingFragmentation` / …（observed，非 spec） |
...
#### 4.6.2 终态：`status.plan`（DryRun/Execute 同一字段 · 易读 + 不失详情）

> **权威 schema**：见 [proposal §5.2](./repack-runtime-defragmentation.md#52-repackrun-api) 与本文 §12。本节示例已对齐封版口径。

DryRun 与 Execute **共用同一 `status.plan`**（一个 Run 只有一种 mode，只填这一个字段）。「值不值得整理」不放 summary，由 `conditions[Complete].reason` 收口。三层渐进披露：

> 1. **`kubectl get` 列**（一行结论，§4.6.2.0）：MODE / PHASE / FREED。
...
> **空结论也清晰**：无收益时 `moves: []`、`summary.freedNodeCount: 0`、`message: "无需整理：…"`，`conditions[Complete].reason` 为 `NoFragmentation`（本就干净）或 `BelowGoalThreshold`（有碎片但低于目标门控，`fragBeforePercent` 仍照填）——一眼区分「没碎片 / 整不动 / 系统出错(phase=Failed)」。
...
NAME                       MODE      PHASE       FREED   AGE
pool-a100-dryrun-202606..  DryRun    Succeeded   2       5m
pool-a100-exec-202606..    Execute   Succeeded   1       2m
...
| FREED | `.status.plan.summary.freedNodeCount` | 腾出整机数（主收益） |
| AGE | `.metadata.creationTimestamp` | |

> 「值不值得整理」看 `conditions[Complete].reason`（`RepackRecommended`/`NoFragmentation`/`BelowGoalThreshold`/`Executed`），不再有 VERDICT 列；FRAG「前→后」需拼两字段，printer JSONPath 取不了，交 UI/`vcctl` 侧合成。
...
│       conditions[Complete].reason ∈ {RepackRecommended, Executed, NoFragmentation, BelowGoalThreshold}
└── plan (RepackPlan)          DryRun=预测 / Execute=已执行，同一字段
    ├── summary                【第2层】纯度量
    │   ├── fragBeforePercent / fragAfterPercent   int32 0-100
    │   ├── freedNodeCount     int32   腾出整机数（printer 列）
    │   ├── movedCardCount     int64   搬走卡数合计
...
| **纯计划、同构** | `moves` 是纯计划，DryRun/Execute 同结构；Execute 实际落点/绑定看 `nominations[].phase` + `summary`（结果导向，漂移不纠正） |
...
| **数组封顶（`maxItems`）** | `status` 进 etcd（单对象 ~1.5MB）。`moves`/`pods`/`freedNodes`/`nominations` 设 `maxItems`；超限只写 `summary` + 截断 + Event 指向导出 |
...
> **不设内部 `formatVersion`**：`plan` 是**类型化 status 子结构**，schema 演进由 **CRD apiVersion 单一治理**（`v1alpha1`→`v1beta1`→…）。K8s status（Pod/Job/PodGroup）均无并行版本号先例。

| 层级 | 机制 |
|------|------|
| **唯一版本源 = CRD apiVersion** | 破坏性改 plan 形状 = 升 CRD 版本（走转换 webhook），不在 status 内自管版本 |
| **只做加法** | 同一 CRD 版本内：新字段 `optional`+`omitempty`，不删/不改名/不改类型；旧客户端忽略未知字段 |
| **核心字段（同版本内稳定）** | `summary`、`moves`、`freedNodes`、`relief` — 保证存在（可能为空数组） |
| **终态不变性** | `Succeeded` 后不再 PATCH plan；升级集群后旧 Run 保留当时 snapshot |

#### 4.6.3 Execute 终态：`status.plan` + `nominations`

Execute 与 DryRun **同一 `status.plan` 结构**（`moves` 是纯计划）；额外：`conditions[Complete].reason: Executed`，并写 **`status.nominations[]`**（durable 落点提名，reconciler 按 §5.2.2 落点身份契约消费）。实际落点/绑定看 `nominations[].phase`、实际腾空看 `summary.freedNodeCount`。
...
status:
  ...
  conditions:
    - { type: Complete, status: "True", reason: Executed, lastTransitionTime: "2026-06-09T10:08:42Z" }
  plan:
    summary:
      fragBeforePercent: 42
      fragAfterPercent: 31            # 实际略逊 DryRun 预估（运行期状态变化）
      freedNodeCount: 1
      movedCardCount: 35
...
**用户 UI / CLI 流程**：读 DryRun `status.plan` → 人工选定 job/node → 新建 Execute Run 填 `spec.scope` → 读 Execute `status.plan` + `nominations`。
...
4. **结果导向**：Execute 完成后核对实际落点 vs plan，漂移如实反映在 `status.plan.moves` 与 `nominations[].phase`，**不强行纠正**。成败看「pending 是否解开」，而非「落点逐一吻合」。
...
- 对每类目标加速资源 `R`（GPU/NPU/…，由 `goals[].resource` 指定）：`M_R` = scope 内**提供 R 的节点数**（`Allocatable[R]>0`）；`B_R` = 其中**当前被 R 任务占用的节点数**；`A_R` = 承接同一批 R 任务的理论最优节点数（下见闭式）。值越小，该资源碎片越少。
...
candidatePlans = 枚举（域 × victim Bundle 组合，受 maxCandidatePlans 上限）
  每个 plan 仍由 ValidatePlan 验证可行（INV-RESCHED）
...
- **复用既有引擎**：`ValidatePlan`（内含 `Domain.Feasible`）驱动 `Statement` / predicate / nodeorder；扩展点只包在它**外围**的策略层，风险可控。
...
**接口与注册表**（与 scheduler `framework.Action` 同形 `Name()`+`Execute`；但跑在 repack 自己的 `ActionContext` 黑板上，便于阶段间传递 in-flight 的 `Plan`/`Report`）：

```go
type Action interface { Name() string; Execute(*ActionContext) }

type ActionContext struct {                 // 共享黑板（CRD/framework 双解耦）
    Snapshot  Snapshot                       // 集群视图
    Params    EngineParams                   // 由 runtime 从 RepackRun.spec 解析
    Algorithm string                          // drain|concentration（§4.16.6）
    Mode      string                          // DryRun | Execute
    Apply     func(*RepackPlan) error         // Execute 提交器（Statement，runtime 注入）
    Plan      *RepackPlan                     // 输出：选定 plan
    Report    *Report                         // 输出：summary
    Err       error                           // 首个错误即停流水线
}

func RegisterAction(name string, f func() Action)         // 注册
func RunActions(names []string, ctx *ActionContext) error // 有序执行，遇错即停
func DefaultActions() []string { return []string{"repack"} } // P0 流水线
```

**P0 唯一 action `repack`** = 度量 + 规划(planner A/B) + 出报告，Execute 再经 `ctx.Apply` 落子（即 §4.16.3 那一个 pass）。
...
> **关键解耦**：`ActionContext` **不含 CRD 类型**（用 `EngineParams`/`Snapshot`）、**不含 framework**（Execute 提交走注入的 `ctx.Apply` 闭包），故 action 层零 scheduler 依赖、可纯 `fakeSnapshot` 单测。多 action 间**通过黑板传 `Plan`/`Report`**（对齐 scheduler action 经 Session 传状态）。配置形态拟 `repack.actions: ["repack"]`（P0），未来 `["relief","repack","simulate"]`——**与 scheduler `actions:` 行同款有序可配**，但默认固定单 `repack`、P0 不开放乱序以避免半成品编排。
...
#### 4.16.6 算法级可插拔：把方案 A / B 做成两个 planner 插件（配置选名）

整理算法本身也是**可插拔的**——方案 A（`orchestrator.go`）与方案 B（`consolidate.go`）都形如 `(nodes, opt) → (*RepackPlan, bool)`，**共用同一执行底座**（碎片度量 §4.12 / 可调度性 §4.14.2 / 扰动评分 §4.16.5 / `framework.Statement` 落盘 §4.14.1；即 §4.17.0 两张时序图"外层逐字相同"那部分）。差别只在内层搜索范式，故可抽一层 **`Planner` 插件接口 + 注册表**，**配置选名即换算法，核心算法零改动**。
...
> 下列 4 张图把两套算法落到**可运行的引擎流程**上：**外层**（CR 驱动、Session 快照、`framework.Statement` 沙箱试算→Commit、`NominatedNodeName` 落盘）两方案**完全一致、共用底座**；**差别只在内层 planner**（A=以节点为单元的构造式贪心 `BuildPlan`，B=以负载为单元的集中度爬山 `Consolidate`）。所用 Volcano 机制：`Session`(Nodes `FutureIdle` / Jobs PodGroup)、`PrePredicateFn`/`PredicateFn`(落点校验=INV-RESCHED)、`framework.Statement`(`Evict`/`Pipeline`/`Commit`/`Discard` 干跑沙箱)、`pod.status.NominatedNodeName`(执行产物)。
...
sequenceDiagram
    ...
    participant Sim as ValidatePlan
...
| 模拟 | `ValidatePlan`（`Statement` 沙箱，§4.14.1） | repack 自有；做 INV-RESCHED victim 重落校验（§4.14.2） |
| 提交 | `Statement.Save/Recover` | 同左，**外加收益门控**（§4.13） |
...
// status 产物：DryRun 与 Execute 合并为同一字段 status.plan（§4.6）。
// DryRun=预测计划；Execute=已执行计划。单资源/Run：碎片率用整数百分比（0-100），
...
type RepackPlan struct {
    ...
    FreedNodes []string            `json:"freedNodes,omitempty"` // 计划腾空的节点名（实际腾空数看 summary.freedNodeCount）
    Relief     []RepackReliefEntry `json:"relief,omitempty"`     // P1：RepackReliefEntry{namespace, podGroupName, relieved}
}
...
type RepackSummary struct { // 纯度量，无 verdict
    ...
    MovedCardCount    int64              `json:"movedCardCount,omitempty"`    // 搬走的卡数总量（反范式计数）
...
// 同构）。Execute 实际落点/绑定交由 nominations[].phase + summary 表达，不逐 pod 记 outcome/
// actualNode（结果导向：漂移不纠正、成败看聚合腾空与 relief）。
type PodMove struct {
...
// 删 role（可读性；身份匹配的 role 只留在 nominations[]）/moveKind（镜像 P1 的 bundlePolicy，
// P0 恒 WholeGroup）/disruptionScore（内部打分、无对外量纲）；原 pods 计数改为 pods[] 逐 pod 明细
// （fromNode/toNode 本质逐 pod，见 PodMove）。
// FreedNode 结构塌缩为 freedNodes []string：actuallyFreed 可由 summary.freedNodeCount + nominations 表达，
// 且「节点保持空」并非成功判据（成功判据是 relief——pending gang 被解开）。
...
type RepackRunStatus struct {
    ...
    Conditions     []metav1.Condition `json:"conditions,omitempty"`     // Queued/Progressing/Complete/Failed/Cancelled（准入=CEL，无 Admitted）
    Plan           *RepackPlan        `json:"plan,omitempty"`           // DryRun=预测 / Execute=已执行（同一字段）
