...
The implementation never detaches or replaces BPF programs owned by another component. The packaged `bwm_prio_kern.o` is loaded directly from the agent image, while `bwm_tc.o` continues to be installed on the host for the CNI. The agent container receives the BPF, network administration, and resource-limit capabilities required to load and attach the cgroup program. Its container-level seccomp profile is `Unconfined` because containerd's `RuntimeDefault` profile on openEuler blocks the `bpf()` syscall even when `CAP_BPF` is present; the other Volcano containers keep the existing pod-level default profile.
