...
The agent selects the backend from `CgroupManager.GetCgroupVersion()`. On cgroup v2, each managed Pod owns one BPF collection and cgroup link. The priority map is populated before the program is attached so packets never observe an uninitialized class. Subsequent Pod updates only update the existing map. Pod deletion, feature disablement, and agent shutdown close the link and collection. Agent restart does not require pinned state: closing the old process removes its links, and the informer initial list recreates them. When Network QoS transitions from disabled back to enabled, the handler also reconciles the ready, non-`hostNetwork` Pods on its node so their transient cgroup v2 links are restored without a workload restart or update.

The implementation never detaches or replaces BPF programs owned by another component. The packaged `bwm_prio_kern.o` is loaded directly from the agent image, while `bwm_tc.o` continues to be installed on the host for the CNI. When `NetworkQoS` (or `*`) is listed in `agent_supported_features`, Helm automatically gives the agent container the BPF, network administration, and resource-limit capabilities required to load and attach the cgroup program. Its container-level seccomp profile is then `Unconfined` because containerd's `RuntimeDefault` profile on openEuler blocks the `bpf()` syscall even when `CAP_BPF` is present. Deployments that do not support NetworkQoS retain the existing restricted agent context, and the other Volcano containers keep the pod-level default profile.
