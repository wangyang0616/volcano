...
> Chinese version: [组间拓扑亲和用户使用指南](./how_to_use_group_topology_affinity_zh.md)
...
This is **not** Kubernetes `PodTopologySpread.topologyKey` (a Node label key). Here you choose a **HyperNode tier** at which `Domain_T` is computed. See [design decision 7](../design/group-topology-affinity.md#ad-7组间层级命名与-kubernetes-topologykey) in the design doc.
